  ...
  if (orderNumber < relationTreeColumnNameOrderList.length) {
    ...
    currentCategorySet.forEach((category: any) => {
      ...
      if (orderNumber === relationTreeColumnNameOrderList.length - 1) {
        ...
        const measureValueListOfRowList: (string | number | null)[] =
          currentCategoryRowList.map((row) => row[title]);
        const generalMeasureValueListOfRowList = filterOutListEmptyValues(
          measureValueListOfRowList
        ) as (string | number)[];
        value = getValueListCalculateValue(
          generalMeasureValueListOfRowList,
          calculateType
        );
      }

      // 加總所有子類節點的值
      const childColumnRelationTreeMap =
        getColumnRelationTreeWithMeasureColumnMap(
          currentCategoryRowList,
          relationTreeColumnNameOrderList,
          measureAnalysisColumn,
          orderNumber + 1
        );
...
const getSunburstDataListByColumnRelationTreeMap = (
  ...
  columnRelationTreeMap.forEach((columnRelationTreeNode, key) => {
    ...
    result.push({
      name,
      value,
      children: getSunburstDataListByColumnRelationTreeMap(
        childColumnRelationTreeMap
      ),
    });
  });
...
export const getBasicSunburstChartOptions = (
  ...
  let relationTreeColumnNameOrderList = categoryAnalysisColumnList.map(
    (column) => column.title
  );
  let categoryColumnRelationTreeMap = getColumnRelationTreeWithMeasureColumnMap(
    rowList,
    relationTreeColumnNameOrderList,
    measureAnalysisColumn
  );
  // set new series data
  newSeries.data = getSunburstDataListByColumnRelationTreeMap(
    categoryColumnRelationTreeMap
  );
