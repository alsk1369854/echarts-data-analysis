...
export const getBasicFunnelChartOptions = (
  ...
  const calculateAnalysisColumn =
    createCalculateAnalysisColumn(calculateColumn);
  const {
    title: calculateColumnTitle,
    calculateType: calculateColumnCalculateType,
  } = calculateAnalysisColumn;
  ...
  categoryCorrespondsCalculateColumnValueListMap.forEach(
    (calculateColumnValueListMap, category) => {
      const categoryCorrespondsCalculateColumnValueList =
        calculateColumnValueListMap.get(calculateColumnTitle);
      if (categoryCorrespondsCalculateColumnValueList) {
        const filterOutNullCategoryCorrespondsCalculateColumnValueList =
          filterOutListEmptyValues(
            categoryCorrespondsCalculateColumnValueList
          ) as (string | number)[];
        newLegend.data.push(category);
        newSeries.data.push({
          name: category,
          value: getValueListCalculateValue(
            filterOutNullCategoryCorrespondsCalculateColumnValueList,
